...
function App() {
  ...
  useEffect(() => {
    if (stage === 'voyage' && voyageProgress >= 0.999) {
      const timer = window.setTimeout(() => {
        setStage('arrived');
        const finalStop = activeRoute.stops[activeRoute.stops.length - 1];
        setSelectedStop(finalStop ?? null);
  ...
  const navigateStory = (direction: -1 | 1) => {
    ...
    const nextIndex = (index + direction + activeRoute.stops.length) % activeRoute.stops.length;
    handleSelectStop(activeRoute.stops[nextIndex]);
  };
    ...
    <main className={`app-shell game-shell game-shell--${stage} ${introOpen ? 'game-shell--intro' : ''}`}>
      ...
      <header className="game-topbar">
        <div className="game-brand">
          <span className="game-brand__mark">ᚠ</span>
          <div><span>Viking Chronology</span><small>Историческая экспедиция · 750–1021</small></div>
        </div>
        <div className="game-topbar__center">
          <span><History size={14} /> {Math.round(timelineYear)}</span>
          <strong>{selectedChapter.title}</strong>
          <span><ShieldCheck size={14} /> мораль {morale}</span>
        </div>
        <div className="game-topbar__actions">
          <button type="button" onClick={() => void audio.toggle()} aria-label="Переключить звук">
            {audio.enabled ? <Volume2 size={18} /> : <VolumeX size={18} />}
          </button>
          <button type="button" className="mobile-game-menu" onClick={() => setMobilePanelOpen((value) => !value)} aria-label="Открыть управление">
      ...
      <div className="game-timeline-wrap">
        <Timeline
          ...
          year={timelineYear}
          playing={stage === 'voyage'}
          playbackSpeed={1}
          onYearChange={() => undefined}
          onTogglePlaying={() => undefined}
          onReset={returnToCouncil}
      ...
      {!audio.enabled && !introOpen && (
        <button type="button" className="sound-invitation" onClick={() => void audio.toggle()}>
          <Headphones size={18} /><span><strong>Включить звуковой мир</strong><small>море, дерево, ветер и музыкальная реконструкция</small></span>
        </button>
      )}
      ...
      {introOpen && (
        <section className="cinematic-intro">
          <div className="cinematic-intro__veil" />
          <div className="cinematic-intro__content">
            <span className="cinematic-intro__runes">ᚠ ᚢ ᚦ ᚬ ᚱ ᚴ ᚼ ᚾ ᛁ ᛅ ᛋ</span>
            <p className="eyebrow">Историческая 3D-игра-хронология</p>
            <h1>Пути северных мореходов</h1>
            <p className="cinematic-intro__lead">Начните во фьорде. Соберите совет. Подготовьте корабль. Пройдите исторический коридор и отделите свидетельства от реконструкции.</p>
            <div className="cinematic-intro__facts">
              <span><Compass size={16} /> 3 экспедиции</span>
              <span><Anchor size={16} /> 19 исторических точек</span>
              <span><ShieldCheck size={16} /> русские субтитры</span>
            </div>
            <button type="button" onClick={() => { setIntroOpen(false); if (!audio.enabled) void audio.toggle(); }}>
              <span>Войти в хронику</span><Compass size={19} />
            </button>
            <small>Лучший опыт: наушники · WebGL 2 · горизонтальная ориентация на телефоне</small>
          </div>
        </section>
      )}
    </main>
