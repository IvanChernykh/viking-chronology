...
export function latLonToUv(lat: number, lon: number): THREE.Vector2 {
  const u = THREE.MathUtils.clamp(
    THREE.MathUtils.mapLinear(lon, MAP_BOUNDS.west, MAP_BOUNDS.east, 0, 1),
    0,
    1,
  );
  const v = THREE.MathUtils.clamp(
    THREE.MathUtils.mapLinear(lat, MAP_BOUNDS.south, MAP_BOUNDS.north, 0, 1),
    0,
    1,
  );
...
export function createFlatMapTexture(width = 2048): THREE.CanvasTexture {
  ...
  land.addColorStop(0, '#46513f');
  land.addColorStop(0.42, '#626448');
  land.addColorStop(0.72, '#4b573f');
  land.addColorStop(1, '#343f35');
  ...
  for (let index = 0; index < count; index += 1) {
    ...
    context.fillStyle = random() > 0.55
      ? `rgba(238, 215, 159, ${0.018 + random() * 0.035})`
      : `rgba(0, 16, 18, ${0.03 + random() * 0.05})`;
  ...
  glow.addColorStop(0, 'rgba(220, 187, 116, 0.16)');
...
export function makeGroundCurve(
  ...
  const bend = Math.min(0.72, length * 0.11) * bendDirection;
