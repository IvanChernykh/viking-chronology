...
interface WorldAtlasObjects extends Objects<GeoJsonProperties> { countries: Objects<GeoJsonProperties>[string]; }
...
export function latLonToUv(lat: number, lon: number): THREE.Vector2 {
  return new THREE.Vector2(
    THREE.MathUtils.clamp(THREE.MathUtils.mapLinear(lon, MAP_BOUNDS.west, MAP_BOUNDS.east, 0, 1), 0, 1),
    THREE.MathUtils.clamp(THREE.MathUtils.mapLinear(lat, MAP_BOUNDS.south, MAP_BOUNDS.north, 0, 1), 0, 1),
  );
}
function projectPosition([lon, lat]: Position, width: number, height: number): [number, number] {
  return [THREE.MathUtils.mapLinear(lon, MAP_BOUNDS.west, MAP_BOUNDS.east, 0, width), THREE.MathUtils.mapLinear(lat, MAP_BOUNDS.north, MAP_BOUNDS.south, 0, height)];
}
...
function countries(): FeatureCollection<Geometry, GeoJsonProperties> {
  return feature(typedAtlas, typedAtlas.objects.countries) as unknown as FeatureCollection<Geometry, GeoJsonProperties>;
}
export function createFlatMapTexture(width = 2048): THREE.CanvasTexture {
  ...
  const ocean = context.createLinearGradient(0, 0, 0, height); ocean.addColorStop(0, '#0d2225'); ocean.addColorStop(0.48, '#102d31'); ocean.addColorStop(1, '#091b1e');
  ...
  const land = context.createLinearGradient(0, 0, width, height); land.addColorStop(0, '#46513f'); land.addColorStop(0.42, '#626448'); land.addColorStop(0.72, '#4b573f'); land.addColorStop(1, '#343f35');
  context.fillStyle = land; context.fill('evenodd'); context.strokeStyle = 'rgba(223, 197, 132, 0.56)'; context.lineWidth = width >= 1600 ? 1.8 : 1.15; context.stroke(); context.restore();
  ...
  const glow = context.createRadialGradient(width * 0.67, height * 0.24, 8, width * 0.67, height * 0.24, width * 0.34); glow.addColorStop(0, 'rgba(220, 187, 116, 0.16)'); glow.addColorStop(1, 'rgba(0, 0, 0, 0)'); context.fillStyle = glow; context.fillRect(0, 0, width, height); context.restore();
...
export function makeGroundCurve(start: { lat: number; lon: number }, end: { lat: number; lon: number }, seed = 0): THREE.CatmullRomCurve3 {
  const a = latLonToMap(start.lat, start.lon, 0.18); const b = latLonToMap(end.lat, end.lon, 0.18); const direction = b.clone().sub(a); const length = direction.length();
  const perpendicular = new THREE.Vector3(-direction.z, 0, direction.x).normalize(); const bend = Math.min(0.72, length * 0.11) * (seed % 2 === 0 ? 1 : -1);
  return new THREE.CatmullRomCurve3([a, a.clone().lerp(b, 0.34).addScaledVector(perpendicular, bend), a.clone().lerp(b, 0.68).addScaledVector(perpendicular, bend * 0.72), b], false, 'centripetal', 0.45);
}
export function createLandMaskTexture(width = 1024): THREE.CanvasTexture {
  const height = Math.round(width * (MAP_DEPTH / MAP_WIDTH)); const canvas = document.createElement('canvas'); canvas.width = width; canvas.height = height;
  const context = canvas.getContext('2d'); if (!context) throw new Error('Canvas 2D context is unavailable.'); context.fillStyle = '#000'; context.fillRect(0, 0, width, height);
  ...
  const texture = new THREE.CanvasTexture(canvas); texture.colorSpace = THREE.NoColorSpace; texture.minFilter = THREE.LinearFilter; texture.magFilter = THREE.LinearFilter; texture.needsUpdate = true; return texture;
}
